  ...
  <div class="content">
      ...
      <Row class="queryline">
        ...
        <Col span="3" offset="1">
          <Button
            ...
            icon="ios-search"
            long
            @click="getQueriedWaterQualities(1)"
            >查询</Button
          >
        </Col>
      </Row>
    ...
    <div class="data_table">
      <Table
        ...
        :columns="columns"
        :data="showWaterQualities"
        @on-sort-change="sortBydate"
      >
        <template slot-scope="{ row, index }" slot="action">
          <Button
            ...
            >编辑</Button
          >
          <Button type="error" size="default" @click="remove(index)"
            >删除</Button
          >
        </template>
      </Table>
    ...
    >
      ...
      <Row class="editLine">
        ...
        <Col span="6">
          <Input
            v-model="chosenWaterQuality.ph"
            :placeholder="chosenWaterQuality.ph.toString()"
          ></Input>
        </Col>
        <Col span="4" offset="1" style="text-align: center">
          <span>溶解氧</span>
        ...
        <Col span="6">
          <Input
            v-model="chosenWaterQuality.do"
            :placeholder="chosenWaterQuality.do.toString()"
          ></Input>
        </Col>
      </Row>
      ...
      <Row class="editLine">
        ...
        <Col span="6">
          <Input
            v-model="chosenWaterQuality.nh3N"
            :placeholder="chosenWaterQuality.nh3N.toString()"
          ></Input>
        </Col>
        <Col span="4" offset="1" style="text-align: center">
          <span>温度</span>
        ...
        <Col span="6">
          <Input
            v-model="chosenWaterQuality.waterTemperature"
            :placeholder="chosenWaterQuality.waterTemperature.toString()"
          ></Input>
        </Col>
      </Row>
...
export default {
  ...
  data() {
    return {
      waterQualities: [], //获取 有多少分页
      showWaterQualities: [], //获取  历史数据
      stationList: [], //获取  监测站  数据
  ...
  methods: {
    ...
    getStartAndEndTime() {
      ...
      this.endDate = new Date(
        this.waterQualities[this.waterQualities.length - 1].date
      );
    ...
    getAllWaterQualities() {
      ...
      this.axios.get("/waterquality/all").then(function(response) {
    ...
    getQueriedWaterQualities(mode = 2) {
      ...
      this.axios
        ...
        .then(function(response) {
          ...
          _this.showWaterQualities = _this.waterQualities.slice(
    ...
    updateWaterQuality() {
      ...
      this.axios({
        ...
        data: _this.qs.stringify({
          ...
          waterTemperature: _this.chosenWaterQuality.waterTemperature,
          date: new Date(this.chosenWaterQuality.date).Format(
            "yyyy-MM-dd hh:mm:ss"
          ),
    ...
    changePage(index) {
      ...
      this.showWaterQualities = this.waterQualities.slice(
        (index - 1) * 10,
        index * 10
      );
    },
    ...
    remove(index) {
      ...
      this.axios
        .post("/waterquality/delete/" + this.chosenWaterQuality.id.toString())
        .then(function(response) {
          if (response.data.status === "success") {
